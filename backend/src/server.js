...
app.get('/api/packs/:uuid', async (req, res) => {
...
app.get('/api/marketplace', async (req, res) => {
  try {
    ...
    res.json(await Promise.all(rows.map((row) => transformerLigneAnnoncePourVue(row))));
  } catch (error) {
...
async function recupererImageJoueurCachee(playerId, imageUrl) {
  try {
    ...
    const contentType = remoteRes.headers.get('content-type') || await trouverContentTypeDepuisExtension(extension);
